...
\newcommand{\methodoverview}[1]{%
  ...
  \begin{tikzpicture}[
    ...
    \node[moTitle] at (8.05,5.31)
      {\molabel{2. Full-pool retrieval and caching}{2. 完整候选池检索与缓存}};
    \node[moTitle] at (6.00,2.51)
      {\molabel{3. Spectrum-conditioned relative reranking}{3. 谱图条件相对重排序}};
    ...
    % Stage 2: score the supplied candidate library and retain a reusable full pool.
    ...
    \node[moBox, text width=1.12cm] (topk) at (6.60,4.30)
      {\molabel{Sort and keep}{排序并保留}\\top-$K=256$};
    \node[moOutput, text width=1.70cm] (cache) at (8.25,4.30)
      {\molabel{Cached full pool}{缓存完整候选池}\\$\{\mathbf z_{m,i},b_i\}$, $i=1{:}256$};
    ...
    % Stage 3: coarse pointwise score followed by relative candidate preferences.
    \node[moBox, text width=3.12cm, minimum height=0.78cm] (features) at (1.72,1.55)
      {\molabel{Pair features}{候选配对特征}\\[-1pt]
       ...
       $|\mathbf z_s-\mathbf z_{m,i}|,b_i]$};
    \node[moBox, text width=0.90cm] (mlp) at (3.98,1.55)
      {\molabel{Coarse MLP}{Coarse MLP}\\$c_i$};
    \node[moChoice, text width=1.95cm] (relative) at (5.62,1.55)
      {\molabel{Relative pair module}{相对候选关系模块}\\$p_{ij}=-p_{ji}$};
    \node[moBox, text width=1.12cm] (head) at (7.20,1.55)
      {\molabel{Relation}{关系}\\$r_i$};
    \node[moBox, text width=1.42cm] (residual) at (8.82,1.55)
      {\molabel{Final score}{最终分数}\\$q_i=c_i+\beta r_i$};
    ...
    \draw[moArrow] (mlp) -- (relative);
    \draw[moArrow] (relative) -- (head);
    ...
    \draw[moArrow, draw=blue!60!black]
      ...
      {\molabel{base path}{基础路径}: $\alpha b_i$} (mlp.north);
    \node[moTrain, text width=4.18cm, minimum height=0.40cm, moSmall] (trainnote) at (5.05,0.45)
      {\molabel{Training: $\mathcal L_{\mathrm{list}}+\lambda_{\mathrm{pair}}\mathcal L_{\mathrm{pair}}
       +\lambda_{\mathrm{spec}}\mathcal L_{\mathrm{spec}}$; no insertion}
      {训练：$\mathcal L_{\mathrm{list}}+\lambda_{\mathrm{pair}}\mathcal L_{\mathrm{pair}}
       +\lambda_{\mathrm{spec}}\mathcal L_{\mathrm{spec}}$；不插入正例}};
