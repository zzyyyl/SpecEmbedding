...
本身是否有价值，后一个问题检验该收益是否必须依赖候选交互。候选池和第一阶段表示保持固定，
使比较集中于第二阶段排序机制。
...
\begin{itemize}
  ...
  \item 在质量和分子式候选协议上系统考察该框架：监督重排序改善基础检索器，但当前证据
  尚不能建立相对候选交互相对 pointwise 打分的独立优势。
\end{itemize}
...
\paragraph{结构解释。}
反向 pair 分数之差使一个有序候选对只有一个偏好方向：交换 $i$ 和 $j$ 时，pair 贡献只改变
符号。这种归纳偏置适合表达“对当前谱图而言候选 $i$ 优于候选 $j$”这样的局部判断，也与训练
...
Transformer \cite{vaswani2017attention}。其历史 forcing 协议和 rank-aware 配置不属于本文正式方法；
详细区别见 Appendix~B。
...
检索指标。Appendix~C 中已有的固定硬件计时仍只是实现成本参考，本轮不新增 timing、显存、吞吐或部署结果。
...
复现边界见 Appendix~D。仓库源码还包含 query-level case exporter，可为指定 cache query 导出候选身份
...
上的 pair 计算为 $O(K_r^2d)$。已有固定硬件的 forward-only 参考保留在 Appendix~C，作为实现
成本观察；它不是端到端 latency、吞吐或部署性能结论。
...
复现或候选感知 alignment 重训。这些限制也界定了本文的两部分发现：框架级重排序观察只在
当前 pilot 内得到支持，而 relative 模块的普适性仍未确定。
...
基础分数路径贡献显著。因此，当前证据支持一个边界明确的监督式重排序框架，并明确了
relative 交互的边界：它是具有结构依据的候选关系机制，但当前 pilot 尚未建立其相对
pointwise 打分的独立且普遍稳健收益。本文方法仍限定于固定的保存嵌入候选协议。
...
后续分析所需的候选规模 forward benchmark 和 query-level case exporter 也已包含在仓库源码中；本稿
没有使用它们新增 timing 曲线或化学案例结果。
