...
Transformer 保留为次级对照。在 MassSpecGym 的质量和分子式候选池以及三个 alignment checkpoint 上，两种监督重排序器都
...
\begin{itemize}
  ...
  \item 在质量和分子式候选协议、三个 alignment checkpoint 以及 30 次 relative 模块消融矩阵
...
受控比较包括余弦基础检索器、容量匹配且无 rank 的 pointwise coarse 打分器、本文提出的
相对 reranker，以及次级 candidate-set Transformer。我们还运行了与 JESTR 类似打分形式的
...
主要表使用官方兼容候选顺序和保存嵌入上的身份标签。canonical 表使用 alignment seed 42、
三个 reranker training seeds（42--44）和最多 20,000 条训练 query；另一个表报告 alignment
seeds 42--44 的相同 pointwise/relative 对照。均值和样本标准差在单个 alignment 内描述
reranker 离散度，跨 alignment 行是 checkpoint 敏感性描述，不是总体重复。本文不声称官方
JESTR 复现、统计显著性或端到端部署性能。

\subsection{主要结果}

\begin{table}[t]
  \centering
  \caption{保存嵌入上的 canonical、官方兼容候选顺序和二维身份无 forcing 评价。学习式结果为
  reranker training seeds 42--44 的均值 $\pm$ 样本标准差；扩展的 cache-local alignment 敏感性
  见表~\ref{tab:alignment_sensitivity_cn}。}
...
在扩展的 cache-local 矩阵中，三个 alignment checkpoint 内的 relative 和容量匹配 pointwise
都改善质量和分子式场景的对应 Base；canonical 的 MassSpecGym 1.3.1-compatible 候选顺序和
二维身份主表使用 alignment-42 保存嵌入。Pointwise 在主要 R@1/MRR 汇总略强，30 次消融也
显示 relative 组件没有统一的独立优势。因此，当前证据支持一个边界明确的监督式重排序框架，并明确了
relative 交互的边界：它是具有结构依据的候选关系机制，但当前证据尚未建立其相对
...
只是内部描述性 estimates，不是总体稳定性声明。$K=20,40,80$ 的评价时候选截断敏感性使用
同一 256-candidate cache，并非按候选规模重新训练。
