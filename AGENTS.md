...
- 正式训练严格使用显式 `cuda:N`，启动前检查 GPU 空闲显存和利用率；繁忙或显存不足时等待，不回退到 CPU、不终止其他任务。CPU 仅用于 synthetic 测试、处理、审计和明确标注的非论文实现 smoke；中断的 CPU 训练工件隔离并排除。
  2026-09-10 用户授权当前优化队列按显存准入：空闲至少10,000 MiB，利用率仅记录、不阻挡
  派发（`max_utilization: 100`），仍30秒轮询、连续120秒及派发前复查。该门槛按batch128
  既有测量设置；改变模型或batch后重新评估，不把显存可用视为算力独占或无OOM保证。

2026-09-11用户授权资源允许时在物理GPU0/1分别运行独立尝试。每卡最多一个本项目训练
阶段；启动前核验固定worktree/commit、模块搜索路径、独立配置、输出目录、可写缓存与
临时目录，记录GPU UUID及进程PID/starttime。运行中不编辑其源码/配置或更新共享Conda
环境。固定输入仅在来源指纹一致且加载路径只读时共享；学习embedding、模型和日志独立。
新并行批次通过`run_with_storage.py --runtime-root`指定独立外部运行缓存目录，细则见
`docs/storage_zh.md`。源码隔离不代表CPU、磁盘或GPU算力无竞争。
...
后续每轮模型实验都须在启动前预先登记：根目录`attempts.md`用一句话记录计划改动与
当前状态，`attempt_<n>.md`写明唯一主要假设、parent及绑定条件、继承参数、缓存/存储影响、
启动依赖和验收标准；准备期间明确标为“准备中，未启动”，不得预填结果。可在当前轮训练时
准备下一轮，完成审计后再绑定parent和单次派发。启动、完成及晋升决定后更新同一条目与
实验卡；未达晋升标准的已完成尝试在`attempts.md`加删除线，保留结果和原因。中断的排队或
运行基础设施修复不冒充失败模型或新增模型attempt。

训练优先级按证据与成本排序，不按编号或实现完成程度机械派发。每份`attempt_<n>.md`
必须解释计划原因、可能提升结果的本地证据（含日志/审计来源）、外部原论文版本与具体
消融位置、协议差异、反证/不确定性、资源成本及延后/恢复条件。测试通过只证明实现可用。
并行尝试可共同绑定一个已完成审计的固定baseline，各自只改变一个主要假设；不得继承
另一批的中途权重或未晋升设置，组合有效改动须另行登记与验证。
