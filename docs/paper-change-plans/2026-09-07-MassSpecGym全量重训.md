...
- 计划约束：本文件沿用仓库模板与原全量重训计划，作为本工作的唯一计划；不另建冲突版本。
  ...
  **提高分子检索 Top-k，MRR 为辅；各项评价指标在 MassSpecGym 上达到或接近现有 SOTA。**
  ...
  2026-09-11用户明确可按GLACIER的方法与结果启发后续实验，本次无需重新核验其具体
  细节；相关适配须登记为待验证假设，不把其分数或消融增幅当作本地已证实收益。
...
- 默认seed42、Mass基础检索，优化胜者成为下一轮incumbent，原始baseline永久保留。
  每个尝试只改变一个主要假设，两卡可按下述隔离规则各跑一个独立方案；基础检索达到
  或接近同协议SOTA后才开展rerank强化，不枚举pointwise开关。
...
- 正式训练/编码/推理显式 cuda:N，按用户授权的显存门槛等待、利用率仅记录；默认在物理
  ...
  2026-09-11用户授权资源及路径隔离条件允许时，两张物理GPU可各运行一个独立尝试；
  不重复占用本项目活跃训练所在卡，不终止其他任务。CPU仅处理、审计及synthetic smoke。
...
可在当前训练时准备下一轮；2026-09-11用户要求按证据重新排序，允许隔离GPU并行。
现行顺序为[A12](../../attempt_12.md)第一优先、[A13](../../attempt_13.md)第二优先，
二者固定绑定已审计A04，各自只改一个主要假设；不依赖另一分支晋升或中途成绩。
[A11](../../attempt_11.md)/[A14](../../attempt_14.md)/[A15](../../attempt_15.md)延后，保留
已有实现与条件预检，旧后继组件不自动部署。各卡须写计划原因、本地证据、原论文版本/
消融位置及协议差异、反证、成本和恢复条件，避免把实现完成当成优化依据。
每次实际进程身份、输入指纹、最终配置和全量预检绑定后才单次派发；并行分支完成后各自
独立审计，组合改动另行预登记，不把排队、准备或部分成绩当成实验完成。
...
- 当次训练运行期间提前完成下一候选的实现、测试、CPU输入核验、实验卡和独立源码准备；
  串行继承时完成审计后再绑定parent；并行独立分支可预先绑定共同已审计baseline。
  这是在2026-09-09提前准备要求上，按2026-09-11授权增加的隔离GPU并行方式。
  不把未审计中途成绩绑定为基线，不绕过GPU门槛、路径隔离或单次派发保护。
...
- 2026-09-11 15:58：A17中央预算预设默认不激活，新增严格继承函数仅变更均匀负例数量，
  拒绝错误parent、结构相似采样、额外权重与非法预算。170项相关CPU回归通过，Ruff、
  绝对路径compileall及diff通过。外部项目根下
  `audits/optimization_a17_config_checks_20260911/receipt.json` SHA为
  `8d5d0bd3151bfe043242baa46fa169167f4be10e197606ba8c495f3137d5987f`，pytest日志SHA为
  `740fa0d006ebc80beb1ad049ace2f92812a253c58a2177051c808142e158f0ca`。
  该检查覆盖配置控制与既有训练/构造回归，不冒充真实32负例GPU成本或正式实验；
  本次未重跑全仓，之前路径失败记录保留。32负例完整实际Dataset、真实训练/审计路径、
  资源评估与固定版本正式预检仍待，没有A17队列，未修改A12/A13/A16固定源码。

- 2026-09-11 15:54：候选预算诊断D27及独立复算完成，使用固定`db5d9c4`源码、已审计
  A04 epoch20全部验证分数与全train候选来源。主诊断主体88.64秒，完整32负例采样及
  逐项核验41.26秒；没有模型forward/backward、GPU或测试分数。外部项目根下
  `audits/negative_budget_gap_20260911/receipt.json` SHA为
  `6be00cc6c31f68aefc0e2941d54713608fa325916371929eb512e3b93331eddd`；原始动态规划经
  26项穷举组合检查，独立卷积/二项式重算全部19,423条query结果一致，194,119条train的
  32负例重新采样哈希为`ccfbf5af1773ff99be5cc578a725853b4c69aac89cc3584991c510c5b212646f`。
  133项文件重新哈希、可用负例数与D16旧工件完全相同，verification SHA为
  `36c5898704947a2f5578f80bb71d456620b3447adb9a3a169deabc0c2c8994c9`。
  初次独立复核因自动审批服务断流未启动；检查脚本CPU范围和排他写路径后同操作获准完成。
  一次只读脚本检查使用系统Python缺少的ast.unparse，改用get_source_segment完成，未改诊断。
  结合A04候选监督收益与MSAlign v1 §5.3/Figure3，预登记A17仅将均匀负例16→32。
  已有PDF SHA为`7395a55141ee7916741b7d9e6d4f42a1d3a03217a36ce6824ac3ff48afd4f26f`；
  原文Formula split/冻结DreaMS和ChemBERTa的收益与成本不继承到本项目。
  已加入未激活的中央预算预设和严格parent配置继承，回归、资源评估与正式全量预检仍待，
  A17没有模型或队列。15:26主机确认A12/A13训练、两批完成审计和A16槽位等待器存活；
  首次主机检查自动审批超时未执行，重试只读检查成功，没有重启任务。

- 2026-09-11 15:06：A16实现以`db5d9c4`提交推送并冻结独立源码；14:57:43 CPU准备
  exit0，实际完整dry-run、194,119条重新tokenize的训练query/目标绑定、A04科学配置
  继承和可写目录隔离通过，未创建正式运行目录。外部项目根下
  `audits/optimization_a16_a04_20260911/receipt.json` SHA为
  `0100c807607d416dbdf9079dbe99564146a90d82818862a9c765ddc10c8f3e25`，preflight SHA为
  `068c28a0d4114896ea264455933a9499f5460013f102a45478d8d8683cb4a91d`；26项输入覆盖7项
  目标文件，实际参数11,000,760，增量1,953,775，未进行GPU测量或模型训练。
  独立衔接准备重算244项固定文件并通过16项生命周期/真实run函数/失败claim保留检查，
  15:04专用tmux单次启动CPU槽位等待器。队列
  `audits/optimization_a16_slot_queue_20260911/`的manifest SHA为
  `e5a2c08c4d53d81dce3de7beab2df34313da3821b419f52739b18d6d87df7840`，launch SHA为
  `c31fbc96caf8dd6fea85fec52f16c4378f3e5130a8dba00b349597952c83f2a6`；15:06独立回执
  verification SHA为`decbf88a612926a95b6b55c44479899eba9d000c0e76775f966b9b5c61c466a6`。
  实际PID3960081/start1459841519，状态`waiting_project_gpu_slot`、CUDA不可见、配置及
  私有运行缓存一致。它绑定A13原入口3895719/start1458877347与trainer3901167/
  start1459020092作为GPU1资源依赖，两者成功结束后才进入固定A16正式入口；对照仍为
  已审计A04，失败保留claim，不重试。A12/A13原trainer仍存活，当前分别完成16/11轮，
  中途完整验证指标更新于各自实验卡，暂无最终晋升结论。A16模型及独立完成审计未启动。

- 2026-09-11 14:50：A16全仓检查1,083通过、1跳过、4失败，原始回执
  `audits/optimization_a16_full_checks_20260911/receipt.json`（相对外部项目根）SHA为
  `a649dd74bc7086c5ac113876be3bcf73da4253b54e9157e07a245f44808dd82b`。
  3个失败为测试运行包装器继承存储根影响相对路径语义，未更改产品存储或训练协议；
  仅在检查子进程移除项目存储语义变量，保留外部TMPDIR及依赖缓存，复验全部相关文件。
  另修正A16卡的两处新增机器路径，补充未声明辅助权重拒绝验证。最终71项通过，仅剩
  4份历史文件的相同路径审计失败；Ruff、绝对路径compileall及diff通过。回执
  `audits/optimization_a16_final_checks_20260911/receipt.json` SHA为
  `8a47d93753a31831c806c8c76c9f86822cc5e1619fc550d9894c062aa30d5274`，pytest日志SHA
  `d2592d8abea2c10c0ebdb05c3b8a74b088c7f53059bbc7420053ef0565d1202e`。
  原失败日志保留；未把补充检查表述为一次全仓全通过。14:46主机只读核验A12/A13训练
  与两批完成审计等待器均存活，未启动重复任务。A16正式真实输入预检和GPU成本仍待。

- 2026-09-11 14:29：A16正式模型构造、实际候选Trainer、训练CLI、运行预检及完整审计
  已接通。复用当前正例分子嵌入计算辅助loss，新增参数纳入原AdamW；目标query绑定、
  每轮全部消费字节重放、主/辅助损失及梯度裁剪均留记录。独立完成审计重载全部保留的
  checkpoint，移除辅助头后的完整合成检索分数逐项相同，未修改原预训练模型或缓存构造源码。
  初次检查211通过、1失败，记录在
  `/data1/zyl/SpecEmbedding/audits/optimization_a16_training_checks_20260911/`；pytest日志SHA
  `a4fb88b349ce4efac2d18eec894bb83c40eca661c56ab92183cfa48ae88fd02a`。
  数值诊断定位于注意力key bias的舍入梯度与AdamW放大效应，未改变模型/优化器，补上
  逐步梯度对照并仅对该已验证退化项设置容差。有效诊断目录
  `/data1/zyl/SpecEmbedding/audits/optimization_a16_gradient_diagnosis_20260911_r2/`，
  numerical_diagnosis SHA `24338f91f5f3b54893a385c2da9e5882e07baa1bf16c641f0b0e36f579200166`。
  首次自动审批服务连接中断未启动进程，确认CPU范围后重试成功；首个诊断随后因合成
  夹具receipt同名未写出报告，原目录与脚本保留，r2另目录分离解决。
  复核218项全部通过，目录
  `/data1/zyl/SpecEmbedding/audits/optimization_a16_training_checks_20260911_r2/`的receipt SHA
  `0b19a0712503a3578f8eed7c69beaa3f61c19d139b1898a720d6587163d6b96d`，pytest日志SHA
  `dad44fc75095f4b5e5fc4b7d7426130273d376f5fdfbbe0153695d5cb4a2c0c2`。随后补齐父模型完整
  配置继承函数并运行全仓检查；真实全量运行预检、GPU成本与派发仍待，没有A16检索成绩。

- 2026-09-11 13:52：A16组件以`0665618`推送并冻结至独立detached源码
  `/data1/zyl/repos/SpecEmbedding-opt-a16-inputs-20260911/`；专用CPU任务13:50:24完成全部
  train目标构建/标量重放/加载，耗时189.12秒，随后独立读回全部稀疏行与来源文件通过，
  194,119条query/token/adduct与既有独立缓存完全一致，346项固定来源哈希不变。
  缓存目录`/data1/zyl/SpecEmbedding/spectrum_target_cache/massspecgym_v15_a16_20260911_topk256/`，
  manifest SHA `09edf11c9a109560be45b266ddd86919ca188f653cb3d4e0cf423a96cdfcb477`，
  audit SHA `296e0cb72fed04d4351253cd7d95d8b2838040d72ed1126eca38fbff70028089`；容量与计数见A16卡。
  运行回执目录`/data1/zyl/SpecEmbedding/audits/optimization_a16_inputs_20260911/`，
  launch SHA `b6a48ff1f9531333e1e2554db1b5d1b2529f7bcb54852d2f72486d7161b99014`，
  receipt SHA `a9b356e5cf74799d0784462e8cccbb821bc35bc78cc6760b5a06c43871b3f4eb`。
  verification SHA `bf70e6695416a57f27379e223f450c52d60b08b439e841f75e27f833acc9affd`，
  保存逐条绑定/归一化和独立文件读回结果，脚本同时归档；CPU输入
  准备不代表正式训练接入。13:52主机核验A12/A13训练及两批完成审计等待器仍活跃，
  A16准备进程已结束，三个固定worktree干净。没有修改论文或启动A16模型。

- 2026-09-11 13:41：A16独立条件辅助头与稀疏目标缓存组件完成，尚未接通正式训练。
  全量query/token绑定、独立标量目标复算、只读mmap与源数据/构造参数/代码指纹失效
  检查已实现，CPU入口要求外部存储且拒绝复用目录。初次90项相关检查通过，回执位于
  `/data1/zyl/SpecEmbedding/audits/optimization_a16_component_checks_20260911/`，pytest日志SHA
  `643473b3b22be13c058dfbc9d2dc618c7d78a57c1968155536846d70f43ef04c`。
  最终补充检查的两个新增测试文件39项通过，含实际1953775参数/29551维输出反向与CPU
  入口；目录`/data1/zyl/SpecEmbedding/audits/optimization_a16_supplemental_checks_20260911/`
  保存最终6项源码指纹与回执，pytest日志SHA
  `72f933938950f2d8bd03c4cdf721c9417589a538adda3cf752afa4826116f6ae`。
  Ruff、绝对路径compileall及diff检查通过；首次相对路径编译因存储包装器切换cwd未找到
  源文件，已修正绝对路径复核，不计为通过。未重跑全仓pytest。全量目标缓存尚待构建，
  正式入口仍拒绝未集成字段，无模型成绩。13:37主机核验A12/A13训练和两批完成审计
  等待进程仍活跃，未触动其固定源码或重复派发。
...
- 2026-09-11 12:28：A10在第30轮正常结束GPU训练，原入口继续CPU全量候选重放，独立
  完成审计等待器仍等待原入口结束；A04仍为incumbent。A12已完成前三轮全量训练/验证，
  实际trainer PID3887701/start1458663413及物理GPU0独立环境已核验，中途成绩/资源见
  A12卡，不提前判断晋升。其独立CPU完成审计队列
  `/data1/zyl/SpecEmbedding/audits/optimization_a12_completion_queue_20260911/`
  已部署并验证实际进程：13项生命周期检查、真实未完成入口拒绝及232项来源哈希通过；
  manifest SHA `41c50b0a54396acfd0083d180f90dd17560f4bfff1ea587163b566bbf48f663f`，
  launch SHA `ddb0e9313b4f7eda57e7ef921f67d281e46b4ec714a2a0f7f0dec5b7e519cf86`，
  `runtime_verification.json` SHA
  `4577b0c469ae662056b4a5411cbd3cecc0cc08abd94e7fb92638f149ece2f72c`。
  A13已冻结e4b6ca5独立worktree并重新核验共同A04分支的23项完整输入与构造；准备目录
  `/data1/zyl/SpecEmbedding/audits/optimization_a13_parallel_a04_20260911/`的
  receipt SHA `4e1bfcbc572d7b7232790bd6b680320d8339f29bb83608377afb1f83725979f7`，
  preflight SHA `776a464531233064fe626ea236d6109bb853c48bb3268d0cbe8b7e6387b819c5`。
  原CPU槽位等待器在确认A10最后GPU子进程成功结束后停止，保留全部锁/日志，停止回执
  `audits/optimization_a13_slot_queue_20260911/gpu_release_replacement.json` SHA
  `aeecb115c3b0cf9ff622a71918a2df58d8981e833b41d1d3a3ec806146ab316e`。
  新启动器曾在派发前误解析proc环境分隔符，确认无新目录/socket/派发后修复并单次启动，
  原因记入独立核验回执；不计失败模型。新派发目录
  `/data1/zyl/SpecEmbedding/audits/optimization_a13_parallel_launch_20260911/`的
  launch SHA `258ef173bd041185813a53bbe3d03835c9e234f40528fb89117694318afee12e`，
  `verification.json` SHA `10b3c9a9d980bc78234fe05a7a8a4fcf0dfb64907f43e5883414efd493cb57df`。
  12:27:52实际入口PID3895719/start1458877347、197项源码配置及23项输入核验通过，
  CPU导入结束、基线评价等待GPU1；尚无A13模型结果，实际trainer出现后才绑定独立完成
  审计。以上均不改变训练协议、现有基线或SOTA目标。
- 2026-09-11：按用户提供的GLACIER方法/结果登记[A16](../../attempt_16.md)，拟以
  训练期观测谱辅助目标增强分子嵌入，保留双塔对比学习与相似度检索。先评估完整train
  目标覆盖、测量条件异质性与低秩预测头成本；未冻结配置、未实现或派发，不预填提升。
  本次不重新核验GLACIER具体细节、下载或复现，也不更新论文结果；本计划保持执行中。

- 2026-09-11 11:45：A12原入口于11:44:01通过GPU0连续准入与最终复查，进入基线
  重编码；实查子进程PID3886707/start1458641994的cwd、源码路径、运行配置、可写
  缓存和GPU UUID全部绑定A12，A10原trainer仍在物理GPU1；再次读回197项源码/YAML。
  同一派发审计目录新增`verification_gpu.json`，SHA
  `222a3ff88b23d2739bcc5adcbef5fa6ca5ec68332b7f9bc9e4e7e6d89f3fcc05`，保留先前等待快照。
  模型训练尚未开始，不能把GPU基线重编码记作新模型结果；状态归档提交`659538c`已推送。
...
- 2026-09-11：以`850fd32`预登记A15后，以`bd4d156`提交独立按头SDPA输出门控及未激活
  ...
  定位错误；修正后121项相关CPU检查全部通过。具体机制、组合和验证边界见A15卡。
  两轮检查根分别为
  `/data1/${USER}/SpecEmbedding/audits/optimization_a15_component_focused_checks_20260911_r1`及`_r2`，
  `receipt.json` SHA分别为
  `8809caaeed6a65344c43614b4a2307b634187baebd2020cfe46d72342a47a636`、
  `18fe7f14f3372a6025c72ff74118790e21599d98a272477badbc0eaa8ffcfc40`；失败日志保留。
  全仓回归1,018通过、1跳过、1个既有内部路径审计失败，仍为原四份历史文件；
  Ruff、compileall及diff检查通过。根目录
  `/data1/${USER}/SpecEmbedding/audits/optimization_a15_component_full_checks_20260911_r1`，
  `receipt.json` SHA为`8c2217cff4b65c784afd036e2a64557175f70d7f70d5d8f6921479373370f17b`，
  `pytest.log` SHA为`56daedf4ef98512ffa7c308a5be26404292a8e3f3446493c633b39150b75c4dd`。
  28项检查日志、来源源码及原论文文件独立读回SHA一致，`independent_verification.json`
  SHA为`02ef5f3c034f201276b1195ea1c4c55eca4291550a05ebc18d64e725b901f682`。
  A15正式构造仍拒绝未接通的新字段；正式接入、全量预检、A14真实依赖/完整审计后的
  最终parent与GPU成本仍待，没有新增训练目录或队列。
  原论文[Gated Attention](https://arxiv.org/abs/2505.06708v1)固定为2025-05-10 arXiv v1，
  不是本地模型成绩或已核验的正式发表记录；10:14:33下载至
  `/data1/${USER}/papers/2505.06708v1_Gated_Attention.pdf`，17页、1,503,818字节，
  SHA为`949c7ed67dea3ddc6b88854cf72631ed2e1f466ad4c824c8aa1286739bcf2359`。
  相邻`2505.06708v1_Gated_Attention.source.json` SHA为
  `3c67f15d2fdab9855e407e16d2cc8a38c6280c787eb7a5dd761f39acfe1e1bb8`；未运行外部模型。
  10:46:53只读主机核验A10原入口/训练、完成审计及A11等待PID/starttime均匹配，
  A10/A11/A13/A14固定源码干净；A11仍等待parent决定。A10前二十三轮完整计数和
  验证分子fresh重编码记录已核验，指标只回填A10卡，不提前晋升或宣称SOTA。
...
- 2026-09-11：A14正式构造、fresh全量候选训练CLI和完整完成审计以`4a2c3bd`提交并推送，
  ...
  完成审计重载选优及全部Pareto权重；原预训练共享结构及固定输入缓存构造未改。
  正式专项初次332通过、8失败，原因是新fixture以未支持字段表示均匀采样；修正测试
  后该文件41项全部通过，生产协议未放宽。两次原始回执与日志分别保留于
  `/data1/${USER}/SpecEmbedding/audits/optimization_a14_formal_checks_20260911_r1`和`_r2`，
  `receipt.json` SHA分别为
  `4171d4be0b6d3dcc1c80292c1aa70caff5f3b556bbbc4a0c6865201af68803df`、
  `f85ada650a2eda70acda6a5c1aa06962a25d693ea13a3412406fd4ffd9c72061`。
  全仓回归根`/data1/${USER}/SpecEmbedding/audits/optimization_a14_formal_full_checks_20260911_r1`，
  `receipt.json` SHA为`6dd973f745f1ed264ca1857681b2600bc32bf7a5f7557df5e884fa399b48408f`；
  972通过、1跳过，唯一失败仍是同四份历史记录的路径审计，Ruff、compileall和diff通过。
  合成实际训练、全部候选重放和严格重载检查通过，不能当作真实模型成绩。
  随后在固定源码运行A04条件全量CPU预检，根目录为
  `/data1/${USER}/SpecEmbedding/audits/optimization_a14_conditional_a04_20260911`；
  `receipt.json` SHA为`508ace464af6e023755029ae40c9644c61fd8a4ea73558300be82762a894153d`，
  `preflight.json` SHA为`f6acdfc9ffa3ac89850a6710c226503d5abb3cb6d9fde2d530d5a7519a271269`。
  09:35:44独立读回19项输入和6份准备文件SHA、仅上下文构造差异及完整分母均通过；
  `verification.json` SHA为`06f5e1c0cee66aaf8619b4d8c826c1e62895069cc5d3c2d7e9e3a416edb477eb`。
  实际构造参数和初值/RNG检查见A14卡；CUDA隐藏、仅dry-run，未创建模型运行目录。
  最终parent仍待A13完整审计，实际依赖、最终绑定/衔接及GPU资源测量未完成。
  09:36:35只读主机核验原A10入口3756121/start1455331545、训练3805869/start1456674958、
  独立审计3810420/start1456826568及A11等待3811702/start1456875263仍存活，原三个
  专用socket/pane身份一致；A11的decision仍为空。A10前十七轮中途记录回填同一实验卡，
  不提前决定晋升，不重启训练或派发重复任务。上述三份测试回执/日志和冻结源码SHA
  另行读回一致（r1已修正的测试fixture按原回执保留），A10/A11/A13/A14固定源码均干净。
...
2026-09-11调度与隔离补充：用户要求按收益证据排序并允许两卡独立尝试，A12/A13固定
A04共同对照，A11/A14/A15延后。11:06仅停止核验过PID/starttime的A11 CPU等待进程，
没有启动模型或删除claim，A10入口/trainer/完成审计保持存活；回执位于
`/data1/zyl/SpecEmbedding/audits/optimization_a11_successor_queue_20260911/priority_deferral_20260911.json`，
SHA-256 `e9424172092f5f2dae02061bb3c89265b6cf081f62123d7bc8ee732c9a9a09c6`。
原A12–A15后继脚本保留为历史准备，不直接部署。各卡补充本地证据、原文具体消融及边界，
不以MSAlign的另设Formula split成绩降低当前同协议SOTA目标。

新增可选独立运行缓存根，并完成99项CPU回归检查（含两个真实Python进程的配置、模块
导入与同名临时文件写入隔离）；Ruff、compileall、diff检查通过。原始回执/日志在
`/data1/zyl/SpecEmbedding/audits/parallel_isolation_checks_20260911/`。正式路径确认绕过
legacy TokenSet写缓存，验证固定图以只读mmap打开并返回tensor副本；实际新批次仍须
冻结源码、生成独立配置并重新全量预检，静态与合成检查不代表已完成实际部署核验。

- 完成日期：尚未完成
- 最终状态：`执行中`；单 baseline 配置与计划已完成，模型优化和 SOTA 达标未完成
- 验证结果：A17全量候选预算诊断与独立重算通过，170项相关配置/训练构造回归及Ruff/compileall/diff通过；本次未重跑全仓。此前A16全仓1,083通过、1跳过、4失败，隔离检查环境后相关71通过，仅剩4份历史文件的相同路径审计失败；原日志均保留于第10节，不以实现或子集概率检查代替实验结果。
- 当前训练状态（2026-09-11 15:26主机核验，15:53原始日志更新）：A10完整审计未晋升，A04仍为incumbent；A12/A13分别在物理GPU0/1训练，已完成21/15轮；A11/A14/A15延后，A16后台等待A13成功结束释放GPU1；A17预登记/输入/配置准备完成，真实训练路径、资源及正式预检仍待，未派发。各轮最新状态集中于[attempts.md](../../attempts.md)，模型优化和SOTA达标未完成。
