...
2026-08-20 release 更新：下文第 7 节中的英文 17 页/中文 16 页是 source commit
`e597d89` 的 2026-08-17 历史审计快照。初稿引用与语言收口后，当前受跟踪 release 从干净
source commit `033ac22` 构建并以 commit `a6063f3` 归档：英文 17 页、479,007 bytes、
SHA-256 `68eaebb36afa5c9fe000b7b948b72080df390f81b1bc1901ff5ca92bfad7dd44`；中文 15 页、
421,262 bytes、SHA-256
`75870b7dde764559f9ba6626b82ecabbe0500a016200a7fd2212051158ffe197`。该更新不改变本报告
“无 P0/P1 阻断、页数为非阻断投稿整理事项”的最终判定。

## 1. 初始审计总体结论（历史，已由第 7 节最终状态取代）

本节只记录初始基线 `df7dec6` 在修复前的时点判断，不表示当前仍有开放阻断；各问题的修复、
复审和最终分级见第 7 节。

初始审计基线已经具备较完整的两阶段 MassSpecGym 实验流水线，核心 reranker 多种子结果、核心组件消融结果和 overlap-clean 处理均有结构化 artifact 支撑。当时论文对证据边界的表述总体谨慎：外部 JESTR/GLMR 数字被标为 `reported`，正文明确不声称严格 SOTA，也承认 self-attention 的独立收益尚未被 checkpoint-robust 证据确认。

但是，该初始版本当时不应直接作为“第三方可复现发布包”或“已完成投稿材料”交付，原因是：
...
3. 英文/中文论文已经编译成功；仓库中 8 月 1 日遗留 PDF 为 15/14 页，而初始审计中间时点的源稿为 16/15 页；`paper/ADMA2026_TODO.md` 与 `docs/project_memory_zh.md` 仍记录 11/10 页，投稿合规记录已过时。后续论文修订和受跟踪 release evidence 为 17/16 页，见第 7 节。
4. 主结果使用本地 exact-SMILES 单正例协议，而 MassSpecGym 官方代码路径默认使用二维 InChIKey 等价并可能存在多个正例；论文已披露差异，但影响尚未量化，因此结果不能直接解释为官方协议等价结果。
5. 主要 reranker 结果固定 alignment seed 42，新增 alignment seeds 的 artifact 虽已存在，但论文主文没有相应的跨 alignment 聚合结果；初始时点的结论应限定为固定 alignment checkpoint 下的 reranker 证据。

初始审计判定（历史，已被最终复审取代）：**研究结论可在明确限定条件下使用；工程复现与投稿材料为有条件通过，发布前需完成当时列出的 P0/P1 修复。**
...
- 8 月 1 日遗留 PDF：英文 `paper/build/main.pdf` 为 15 页，中文 `paper/build/main_cn.pdf` 为 14 页；初始审计中间时点的源稿为英文 16 页、中文 15 页；这不是最终 release evidence，后续受跟踪版本为 17/16 页；
- LaTeX 日志未发现致命错误、未定义引用或未定义交叉引用；仅见 `fontspec` 字体和 `amsmath` 重定义 warning；
- 初始时点的默认环境 `pytest -q`：收集阶段失败；缺少 `torch`、`SpecEmbedding` 根目录导入路径以及多个根目录分析模块；
...
## 3. 初始审计问题清单（历史；P0/P1 均已关闭）

以下分级只表示 `df7dec6` 初始时点的修复优先级，不是当前开放待办。P0/P1 的关闭证据和
最终无阻断结论见第 7 节；仍保留的科学限制已降为明确披露的证据边界。

### 初始 P0-1（已关闭）：测试与运行环境无法直接验证
...
影响：初始时点不能声明测试通过，也不能把测试文件视为第三方可复现的验收入口。部分问题可能只是环境配置问题，但实际失败发生在收集阶段，尚未进入测试逻辑。
...
### 初始 P0-2（已按严格本地协议标注方案关闭）：数据与候选身份协议未与官方评价协议统一

位置：`paper/main.tex:507-514`、`paper/main_cn.tex:410-419`，以及 `prepare_rerank_cache.py`、`eval_rerank.py` 的正例判定逻辑。

现象：初始时点评估按 exact target-SMILES 赋予单一正例；论文同时指出官方 MassSpecGym loader 默认使用二维 InChIKey 等价并可能标注多个正例。该差异会影响 Recall、MRR 和候选覆盖定义，但当时没有 identity-rule sensitivity 数字。
...
### 初始 P1-2（记录问题已关闭；压页为非阻断待办）：论文与项目状态文档的页数记录过时

位置：`paper/ADMA2026_TODO.md:76,160`、`docs/project_memory_zh.md:699-700`。

现象：8 月 1 日遗留构建产物显示英文 15 页、中文 14 页；初始审计中间时点的源稿为英文 16 页、中文 15 页。但文档仍写“11/10 页”，并将英文 11 页标记为已完成。后续受跟踪 release evidence 为 17/16 页，记录问题已经关闭；页数微调按用户决定不阻塞本轮审计。

影响：投稿合规审计会产生错误结论；英文稿已经达到投稿限制上限，任何新增内容、字体变化或编译环境变化都可能超页。

建议：立即更新状态文档为实际页数，记录编译命令、模板和 warning；在最终投稿前确认 PDF 页数和元数据，而不是沿用旧记录。

### 初始 P1-3（已关闭）：主结果的 alignment 不确定性未纳入论文主结论
...
影响：结果稳定性结论的适用范围较窄。初始时点可以支持“固定基础 alignment 下 reranker 的初始化稳定性”，不能支持完整流水线的随机种子稳定性。
...
### 初始 P1-4（已按 reported-only 分表方案关闭）：外部 baseline 未独立复现，跨论文表存在协议不可比风险
...
### 初始 P2-1（已完成口径分离）：论文主表、受控均值和代表性 seed 的统计口径容易混淆
...
### 初始 P2-2（已完成事后验收锁；历史训练边界保留）：开发依赖和版本没有完全锁定
...
### 初始 P2-3（非阻断维护项）：文档中的旧流程与新流程并存

位置：`README.md`、`docs/README_zh.md`、`paper/ADMA2026_TODO.md`、`docs/project_memory_zh.md`。

现象：README 同时介绍早期 spectrum-to-spectrum `.npy` 评估、当时的 alignment/candidate 评估和 reranker；部分路径与环境说明仍服务于旧流程。

影响：新用户无法判断哪个流程生成论文主结果，也容易使用不兼容的 checkpoint、candidate file 或指标协议。

建议：在 README 首部明确“论文主结果复现路径”，将 legacy workflow 单列并标明不用于论文主表；为每种结果给出 artifact 目录、commit 和预期数值。
...
- 英文和中文稿均有可用 PDF；初始审计日志没有发现致命 LaTeX 错误或未定义引用。
- Git 历史未发现已跟踪的模型权重、密钥文件或 `.env` 文件；但 artifact/log 脱敏仍需在公开打包前单独执行。

## 5. 初始审计修改优先级（历史执行清单，非当前阻断）

以下条目用于解释后续修复顺序；当前关闭状态以第 7 节为准。英文页数按用户决定独立延后，
不再作为 P0/P1 或本轮审计阻断。
...
2. 补充参数量、cache 构建、推理延迟和显存信息；若不补，保留相应 limitation。
3. 清理 legacy 文档和过时项目记忆。

## 6. 初始审计研究结论边界（历史）

本节是修复前基线能够支持的最窄结论；后续已经加入三个 alignment 的分层描述性证据，
最终边界和复审结果见第 7 节。
...
这些限制已在论文关键独立阅读位置自包含披露。当时下一步为提交原审计会话再次审计；该复审
现已完成，最终结果见本节后续记录。

### 2026-08-17：第二轮独立审计续修

当时状态：**两项 P1 已完成本地修复，等待第三轮独立复审**。第三轮现已完成并确认本轮
无 P0/P1 阻断项，见下方“第三轮独立审计结论与分级更正”。
